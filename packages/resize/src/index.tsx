...
export const Resizable: React.FC<ResizableProps> = (props) => {
  ...
  const onResizeStart = useCallback(
    (
      ...
      console.log(resizableRef.current, 'resizableRef.current')
    ...
    [resizableRef, window, props, state, cpuSize, bindEvents, parentNode]
  )
  ...
  const onMouseUp = useCallback(
    ...
    [state, resizableRef, props, unbindEvents]
  )

  // 更新尺寸
  const updateSize = useCallback((size: Size) => {
    setState({ width: size.width ?? 'auto', height: size.height ?? 'auto' })
  }, [])
